...
<template>
    <section class="flex flex-col">
        <h3 class="text-2xl text-center">Liste des quartiers</h3>
        <ul>
        ...
        <ul>
          <li v-for="quartierObject in les_quartiers" class="">
            ...
            <li class="pl-5 leading-10   " v-for="commune in data">
                {{ commune?.nom_commune }} -
                <ul>
                    <li class="pl-3" v-for="Quartier in (commune.Quartier)">
                        <RouterLink class="text-red-700" :to='{ name: "/quartier/edit/[[id]]", params: { id: Quartier.id } }'>
                            {{ Quartier.nom_quartier }}
                        </RouterLink>
                        <RouterLink :to='{ name: "/quartier/suppr/[[id]]", params: { id: Quartier.id } }' class="focus-style justify-self-end rounded-md bg-red-500 p-1 ml-5  shadow-sm">
                      Supprimer l'offre
                  </RouterLink>
                    </li>
                </ul>
